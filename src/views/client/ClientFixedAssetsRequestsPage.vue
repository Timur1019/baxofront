...
const fixedAssetsFile = ref(null)
const fixedAssetItems = ref([])

const assetTypes = [
  { value: 'JEWELLERY', labelKey: 'fixedAssets.assetTypeJewellery' },
  { value: 'INVENTORY', labelKey: 'fixedAssets.assetTypeInventory' },
  { value: 'EQUIPMENT', labelKey: 'fixedAssets.assetTypeEquipment' },
]

const units = [
  { value: 'DONA', labelKey: 'fixedAssets.unitDona' },
  { value: 'KG', labelKey: 'fixedAssets.unitKg' },
  { value: 'LITR', labelKey: 'fixedAssets.unitLitr' },
  { value: 'BUKET', labelKey: 'fixedAssets.unitBuket' },
  { value: 'PACHKA', labelKey: 'fixedAssets.unitPachka' },
  { value: 'JUFT', labelKey: 'fixedAssets.unitJuft' },
  { value: 'PORTZIYA', labelKey: 'fixedAssets.unitPortziya' },
  { value: 'M2', labelKey: 'fixedAssets.unitM2' },
  { value: 'M', labelKey: 'fixedAssets.unitM' },
  { value: 'HALF_PORTZIYA', labelKey: 'fixedAssets.unitHalfPortziya' },
  { value: 'CASSETTE', labelKey: 'fixedAssets.unitCassette' },
]
...
function openAddModal() {
  ...
  fixedAssetsFile.value = null
  fixedAssetItems.value = []
  formError.value = ''
...
function addProperty() {
  if (!propertyForm.value.name?.trim() || !propertyForm.value.quantity) {
    return
  }
  const qty = parseFloat(propertyForm.value.quantity)
  if (isNaN(qty) || qty <= 0) return
  fixedAssetItems.value.push({
    assetType: propertyForm.value.assetType,
    name: propertyForm.value.name.trim(),
    quantity: qty,
    unitOfMeasurement: propertyForm.value.unitOfMeasurement || null,
  })
  closePropertyModal()
}

function removeProperty(idx) {
  fixedAssetItems.value.splice(idx, 1)
}

function onFileChange(e) {
  fixedAssetsFile.value = e.target.files?.[0] || null
}

async function submitAdd() {
  if (!form.value.propertyOwnerName?.trim() || !form.value.objectAddress?.trim() ||
      !form.value.ownerPhone?.trim() || !form.value.bankEmployeePhone?.trim() ||
      !form.value.appraisalPurpose?.trim()) {
    formError.value = t('client.formErrorFillRequired')
    return
  }
  if (!fixedAssetsFile.value) {
    formError.value = t('client.fixedAssets.formErrorUploadDoc')
    return
  }
  if (fixedAssetItems.value.length === 0) {
    formError.value = t('client.fixedAssets.formErrorAddProperties')
    return
  }
  ...
  try {
    const dto = {
      requestType: 'FIXED_ASSETS',
      propertyOwnerName: form.value.propertyOwnerName.trim(),
      borrowerInn: form.value.borrowerInn?.trim() || null,
      objectAddress: form.value.objectAddress.trim(),
      ownerPhone: form.value.ownerPhone.trim(),
      bankEmployeePhone: form.value.bankEmployeePhone.trim(),
      appraisalPurpose: form.value.appraisalPurpose,
      fixedAssetItems: fixedAssetItems.value.map(it => ({
        assetType: it.assetType,
        name: it.name,
        quantity: it.quantity,
        unitOfMeasurement: it.unitOfMeasurement || null,
      })),
    }
    const created = await createEvaluationRequestWithDocument(dto, null, null, fixedAssetsFile.value)
    closeAddModal()
    load()
...
function assetTypeLabel(v) {
  return assetTypes.find(a => a.value === v) ? t('client.' + assetTypes.find(a => a.value === v).labelKey) : v
}

function unitLabel(v) {
  return units.find(u => u.value === v) ? t('client.' + units.find(u => u.value === v).labelKey) : (v || '—')
}
</script>
      ...
      <div class="client-requests-page__table-wrap">
        <table class="client-requests-page__table">
          ...
          <tr>
            ...
            <th>{{ t('client.fixedAssets.tablePropertyOwner') }}</th>
          ...
          >
            ...
            <td>{{ item.borrowerName || item.borrowerInn || '—' }}</td>
            <td>
        ...
        <div class="modal-container modal-container--wide">
          <div class="modal-header">
            <h2 class="modal-title">
              ...
              {{ t('client.fixedAssets.modalTitle') }}
            </h2>
            ...
            <div class="modal-form__grid">
              <div class="modal-form__group modal-form__group--full">
                <label class="modal-form__label">
                  {{ t('client.fixedAssets.propertyOwnerName') }} <span class="modal-form__required">*</span>
                </label>
                <input
                    v-model="form.propertyOwnerName"
                    type="text"
                    class="modal-form__input"
                    :placeholder="t('client.fixedAssets.propertyOwnerPlaceholder')"
                    required
                />
              </div>
              ...
              <div class="modal-form__group modal-form__group--full">
                <label class="modal-form__label">
                  {{ t('client.fixedAssets.objectAddress') }} <span class="modal-form__required">*</span>
                </label>
                <input
                    v-model="form.objectAddress"
                    type="text"
                    class="modal-form__input"
                    :placeholder="t('client.fixedAssets.objectAddressPlaceholder')"
                    required
                />
              </div>
              <div class="modal-form__group modal-form__group--full">
                <label class="modal-form__label">
                  {{ t('client.fixedAssets.uploadDocuments') }} <span class="modal-form__required">*</span>
                </label>
                <div class="modal-form__file-zone" @click="$refs.fileInput?.click()">
                  ...
                  <div class="modal-form__file-text">
                    {{ fixedAssetsFile ? fixedAssetsFile.name : t('client.modalFileZone') }}
                  </div>
                  <div class="modal-form__file-hint" v-if="!fixedAssetsFile">
              ...
              <div class="modal-form__group modal-form__group--full">
                <label class="modal-form__label">
                  {{ t('client.fixedAssets.addPropertiesLabel') }} <span class="modal-form__required">*</span>
                </label>
                <div class="modal-form__properties-block">
                  <button type="button" class="client-requests-page__add-props-btn" @click="openPropertyModal">
                    + {{ t('client.fixedAssets.addPropertyBtn') }}
                  </button>
                  <div v-if="fixedAssetItems.length > 0" class="modal-form__properties-list">
                    <div
                        v-for="(it, idx) in fixedAssetItems"
                        :key="idx"
                        class="modal-form__property-row"
                    >
                      <span class="modal-form__property-info">{{ assetTypeLabel(it.assetType) }}: {{ it.name }} — {{ it.quantity }} {{ unitLabel(it.unitOfMeasurement) }}</span>
                      <button type="button" class="modal-form__property-remove" @click="removeProperty(idx)">×</button>
                    </div>
                  </div>
                </div>
              </div>
    ...
    <Transition name="modal">
      <div v-if="showPropertyModal" class="modal-overlay" @click.self="closePropertyModal">
        <div class="modal-container modal-container--narrow">
          <div class="modal-header">
            <h2 class="modal-title">{{ t('client.fixedAssets.addPropertyModalTitle') }}</h2>
            <button class="modal-close" @click="closePropertyModal">×</button>
          </div>
          <form class="modal-form" @submit.prevent="addProperty">
            <div class="modal-form__grid">
              <div class="modal-form__group modal-form__group--full">
                <label class="modal-form__label">{{ t('client.fixedAssets.assetType') }}</label>
                <select v-model="propertyForm.assetType" class="modal-form__select">
                  <option v-for="at in assetTypes" :key="at.value" :value="at.value">{{ t('client.' + at.labelKey) }}</option>
                </select>
              </div>
              <div class="modal-form__group modal-form__group--full">
                <label class="modal-form__label">
                  {{ t('client.fixedAssets.propertyName') }} <span class="modal-form__required">*</span>
                </label>
                <input
                    v-model="propertyForm.name"
                    type="text"
                    class="modal-form__input"
                    :placeholder="t('client.fixedAssets.propertyNamePlaceholder')"
                    required
                />
              </div>
              <div class="modal-form__group">
                <label class="modal-form__label">
                  {{ t('client.fixedAssets.quantity') }} <span class="modal-form__required">*</span>
                </label>
                <input
                    ...
                    min="0"
                    class="modal-form__input"
                    :placeholder="t('client.fixedAssets.quantityPlaceholder')"
                    required
                />
              </div>
              <div class="modal-form__group">
                <label class="modal-form__label">{{ t('client.fixedAssets.unitOfMeasurement') }}</label>
                <select v-model="propertyForm.unitOfMeasurement" class="modal-form__select">
                  <option v-for="u in units" :key="u.value" :value="u.value">{{ t('client.' + u.labelKey) }}</option>
                </select>
              </div>
            </div>
            <div class="modal-form__actions">
              <button type="button" class="modal-form__btn modal-form__btn--secondary" @click="closePropertyModal">
                {{ t('client.modalBack') }}
              </button>
              <button type="submit" class="modal-form__btn modal-form__btn--primary">
                {{ t('client.fixedAssets.addPropertyBtn') }}
              </button>
            </div>
          </form>
...
.client-requests-page__add-props-btn {
  padding: 0.5rem 1rem;
...
.client-requests-page__add-props-btn:hover {
  background: rgba(111, 66, 193, 0.14);
  border-color: var(--color-accent-hover);
}

.modal-form__properties-block {
  display: flex;
  flex-direction: column;
  gap: 0.75rem;
}

.modal-form__properties-list {
  display: flex;
  flex-direction: column;
  gap: 0.5rem;
}

.modal-form__property-row {
  display: flex;
  align-items: center;
...
.modal-form__property-info {
  color: #495057;
}

.modal-form__property-remove {
  width: 24px;
  height: 24px;
  padding: 0;
  border: none;
...
.modal-form__property-remove:hover {
